...
static bool updateOperand(FoldCandidate &Fold,
  ...
  if ((Fold.isImm() || Fold.isFI() || Fold.isGlobal()) && Fold.needsShrink()) {
    ...
    bool HaveNonDbgCarryUse = !MRI.use_empty(Dst1.getReg());
...
static bool hasOneUseInst(const MachineRegisterInfo &MRI, unsigned Reg) {
  int Count = 0;
  for (auto I = MRI.use_instr_begin(Reg), E = MRI.use_instr_end();
       I != E; ++I) {
    if (++Count > 1)
      return false;
  }
...
bool SIFoldOperands::tryFoldClamp(MachineInstr &MI) {
  ...
  if (!ClampSrc || !hasOneUseInst(*MRI, ClampSrc->getReg()))
    return false;
...
bool SIFoldOperands::tryFoldOMod(MachineInstr &MI) {
  ...
  if (OMod == SIOutMods::NONE || !RegOp->isReg() ||
      ...
      !hasOneUseInst(*MRI, RegOp->getReg()))
    return false;
