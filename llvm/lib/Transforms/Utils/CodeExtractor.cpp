...
static void fixupDebugInfoPostExtraction(Function &OldFunc, Function &NewFunc,
  ...
  if (!OldSP) {
    // The old function had no subprogram. Erase any debug info intrinsics the
    ...
    auto NewSP = DIB.createFunction(OldSP->getUnit(), NewFunc.getName(),
                                    NewFunc.getName(), OldSP->getFile(),
                                    /*LineNo=*/0, SPType, /*ScopeLine=*/0,
    ...
    for (Instruction &I : instructions(NewFunc)) {
      if (const DebugLoc &DL = I.getDebugLoc())
        ...
        // Ensure that inlinable calls have some debug location.
        I.setDebugLoc(DebugLoc::get(0, 0, NewSP));
